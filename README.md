...
It is also possible to configure API's URL by setting env variable API_URL, for example, `API_URL=http://localhost:9000 pnpm start`. The default is <http://localhost:8000>
...
```bash
docker run -p 3000:8080 -e API_URL=<api-url-here> -e USE_AXE=<true|false> hauki-admin-ui
```
